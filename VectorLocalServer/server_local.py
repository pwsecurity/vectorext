...
Authentication is validated against GitHub (users.json).
API Keys are stored locally on the user's Desktop for security.
...
GITHUB_USERS_URL = "https://raw.githubusercontent.com/pwsecurity/vectorext/refs/heads/main/users.json"
LOCAL_PORT = 5002
GENAI_HTTP_TIMEOUT_SEC = int(os.getenv("GENAI_HTTP_TIMEOUT_SEC", "90"))

# Path to keys file on User's Desktop
LOCAL_KEYS_PATH = os.path.join(os.path.expanduser("~"), "Desktop", "vector_keys.txt")
...
# Next run, user logs in again → cache is rebuilt from GitHub.
...
def fetch_github_users():
    """Fetch the central users database from GitHub."""
    try:
        logger.info("Fetching users from GitHub...")
        resp = http_requests.get(GITHUB_USERS_URL, timeout=15)
        resp.raise_for_status()
        return resp.json()
    except Exception as e:
        logger.error(f"Failed to fetch users from GitHub: {e}")
        return None

def get_local_keys():
    """Read API keys from local desktop file."""
    if not os.path.exists(LOCAL_KEYS_PATH):
        return []
    try:
        ...
        logger.error(f"Error reading local keys: {e}")
        return []

def save_local_keys(keys):
    """Save API keys to local desktop file."""
    try:
        # Ensure directory exists (though Desktop always should)
        ...
        logger.error(f"Error saving local keys: {e}")
        return False
...
def get_keys_for_user(email):
    """Get API keys for user from local storage."""
    # Always use local keys file for security
    return get_local_keys()
...
def login():
    try:
        ...
        # --- First login this session → call GitHub ---
        logger.info(f"First login for {email} this session. Validating with GitHub...")

        github_data = fetch_github_users()
        if not github_data or "users" not in github_data:
            return jsonify({'error': 'Cannot connect to authentication database on GitHub.'}), 503

        users = github_data.get("users", {})
        ...
        if status != 'approved':
            return jsonify({'error': f"Your account status is: {status}"}), 403
        ...
        cache_data = {
            ...
            'is_expired': False, # Manual check above
            'expiry_date': expiry_str,
            'active_devices': 1,
            'device_changed': False,
            # From profile (GitHub)
            'persona': user_data.get('persona', DEFAULT_PERSONA),
            'api_keys': local_keys, # Use local keys for security
            'model_access': user_data.get('model_access', {m["id"]: True for m in DEFAULT_MODELS_CONFIG}),
            'media_access': user_data.get('media_access', {'images': True, 'files': True, 'audio': True}),
            'message_limit': user_data.get('message_limit', False),
            'auto_pilot': user_data.get('auto_pilot', False),
            'use_server_keys': False, # Security: only use local keys from desktop
            'session_limit': user_data.get('session_limit', 1),
            # System settings
            'system_settings': github_data.get('system_settings', {
                'models_config': DEFAULT_MODELS_CONFIG,
                'models': {m["id"]: True for m in DEFAULT_MODELS_CONFIG}
            }),
            # Timestamp
            'cached_at': datetime.now().isoformat()
        }
        cache_user(email, cache_data)
        logger.info(f"User {email} validated via GitHub and cached. Local Keys: {len(local_keys)}")
...
def signup():
    """Signup is disabled in local mode. Please contact administrator."""
    return jsonify({'error': 'Registration is disabled for local server. Please contact an administrator.'}), 403
...
def update_persona():
    try:
        ...
        return jsonify({'success': True, 'message': 'Persona updated locally for this session.'})
    except Exception as e:
...
def get_user_keys_route():
    try:
        ...
        logger.error(f"Error in get_user_keys_route: {e}")
        return jsonify({'error': 'Server error'}), 500


@app.route('/api/keys/update', methods=['POST'])
def update_user_keys_route():
    try:
        is_valid, email, error_msg = validate_auth_local()
        if not is_valid:
            return jsonify({'error': error_msg or 'Unauthorized'}), 401
        ...
        # Save keys to local desktop file for security
        success = save_local_keys(new_keys)
        if success:
            # Update local cache too
            update_cache_field(email, 'api_keys', new_keys)
            return jsonify({'success': True, 'message': 'API keys saved securely on your Desktop.', 'saved_count': len(new_keys)})
        else:
            return jsonify({'error': 'Failed to save API keys to your Desktop.'}), 500

    except Exception as e:
        logger.error(f"Error in update_user_keys_route: {e}")
        return jsonify({'error': 'Server error'}), 500
...
def extension_version():
    """Version check is disabled in local mode or returns local info."""
    return jsonify({'version': '3.4', 'uploaded_at': datetime.now().isoformat()})
